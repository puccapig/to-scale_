...
shift_footsteps <- function(){
	...
	o_1 <- sample(bottle_cont, 3)
	#print(bottle_)
	print(o_1)
	
	print("shift speed:")
	shift_speed <- readline()
	shift_speed <- as.numeric(shift_speed)
	print("shift forward:")
	shift_forward <- readline()
	shift_forward <- as.numeric(shift_forward)
	
	shift_unit <- shift_forward * shift_speed
	else_seq1 <- sapply(substr(else_seq, shift_unit, nchar(else_seq)), function(x){x[1]})
	else_seq2 <- sapply(substr(else_seq, shift_unit*2, nchar(else_seq)), function(x){x[1]})
	else_seq0 <- c(else_seq1, else_seq2)
	
	par(mfrow = c(2,3), cex = 0.5)
	for(ki in 1:2){
		else_seq <- as.character(else_seq0[ki])
		len_seq <- nchar(else_seq)
	for(k in 1:3){
